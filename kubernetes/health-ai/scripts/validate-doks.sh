#!/bin/bash
# validate-doks.sh — DOKS functional validation
# Adapted from validate-gke.sh — replaces Cloud Logging with kubectl logs.
# Tests are distributed across all 4 services to confirm eBPF monitors the full cluster.
# Usage: ./validate-doks.sh [--context <kubectl-context>]
#
...
# Fetch alerts from eBPF agent stdout after $1 (RFC3339 timestamp).
# Agent logs are plaintext key=value: "... ALERT level=X rule=Y service=Z namespace=W ..."
# Output format: "level=X rule=Y service=Z namespace=W filename=F dst_ip=I"
_fetch_alerts() {
    local since=$1
    $KUBECTL logs "$EDR_POD" -n kube-system --since-time="$since" 2>/dev/null \
    | python3 -c "
import sys, re
...
for line in sys.stdin:
    ...
    print('level={} rule={} service={} namespace={} filename={} dst_ip={}'.format(
        get(line, 'level'), get(line, 'rule'), get(line, 'service'),
        get(line, 'namespace'), get(line, 'filename'), get(line, 'dst')))
"
}

# Poll eBPF agent logs for a pattern anchored to $3 (RFC3339 timestamp), return 0 if found within timeout.
expect_alert() {
    local pattern=$1
    local timeout=${2:-60}
...
# Check that a pattern does NOT appear in eBPF agent logs over a wait window.
no_alert() {
    local pattern=$1
    local window=${2:-20}
    local since
